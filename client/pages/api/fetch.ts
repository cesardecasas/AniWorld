...
export const getDetails = async (id: number | string) => {
  try {
...
export const getMangaDetails = async (id: number | string) => {
  try {
...
export const getSong = async (id: number | string) => {
  try {
...
export const getNews = async () => {
  try {
    const res = await NewsClient.get("reports.xml?id=155&type=anime&nlist=all", {
      headers: {
